...
// Creates a friendly error message from Peggy parser errors
function friendlyMiniError(error, code) {
  const { expected, found } = error;
  
  // Extract what characters were expected
  const expectedChars = expected
    ?.filter(e => e.type === 'literal')
    ?.map(e => e.text) || [];
  
  // Check for common bracket mismatches
  if (expectedChars.includes(']')) {
    return `Missing closing bracket "]" — you opened a "[" but never closed it`;
  }
  if (expectedChars.includes(')')) {
    return `Missing closing parenthesis ")" — check your euclidean rhythm syntax, e.g. bd(3,8)`;
  }
  if (expectedChars.includes('>')) {
    return `Missing closing angle bracket ">" — you started a slow sequence with "<" but didn't close it`;
  }
  if (expectedChars.includes('}')) {
    return `Missing closing brace "}" — you started a polymeter with "{" but didn't close it`;
  }
  if (expectedChars.includes(',')) {
    return `Missing comma "," — euclidean rhythms need format: sound(pulses, steps)`;
  }
  
  // Check for unexpected closing brackets (no matching opener)
  if (found && ']}>)'.includes(found)) {
    const names = { ']': 'bracket', '}': 'brace', '>': 'angle bracket', ')': 'parenthesis' };
    return `Unexpected "${found}" — this closing ${names[found]} doesn't have a matching opener`;
  }
  
  // Check for invalid characters
  if (found && !found.match(/[a-zA-Z0-9\s\[\]<>{}\(\),\.|~\-#\.\^_@!*\/?:]/)) {
    return `Invalid character "${found}" — this character is not allowed in mini notation`;
  }
...
export const mini2ast = (code, start = 0, userCode = code) => {
  try {
    ...
    const friendlyMsg = friendlyMiniError(error, code);
    const err = new Error(`[mini] ${friendlyMsg}`);
    err.region = region;
    err.line = line;
    err.column = col;
    throw err;
...
export const m = (str, offset) => {
  const code = `"${str}"`;
  const ast = mini2ast(code, offset);
