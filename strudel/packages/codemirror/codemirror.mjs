...
import { errorHighlightExtension, highlightError, clearErrorHighlight } from './errorHighlight.mjs';
  ...
  constructor(options) {
    ...
    this.repl = repl({
      ...
      onEvalError: (err) => {
        ...
        if (err.region && this.editor) {
          highlightError(this.editor, err.region[0], err.region[1]);
        }
        replOptions?.onEvalError?.(err);
      },
      onToggle: (started) => {
      ...
      afterEval: (options) => {
        // Clear error highlight on successful eval
        clearErrorHighlight(this.editor);
